    ...
    methods: {
      ...
      async confirmAddress() {
        ...
        try {
          ...
          } else if (erc20.checkSymbol(this.symbol)) {
            if (wallet.extend.ledger) {
              this.rawTx = 'Please confirm tx on your ledger...'
            }
            const token = erc20.getTokenBySymbol(this.symbol)
            const encodedData = erc20.encodeSendData(token, this.address, this.amount)
      ...
      async searchAddToken() {
        ...
        try {
          const tokenInfo = await erc20.fetchTokenInfo(this.addTokenAddress)
      ...
      confirmAddToken() {
        erc20.addCustomToken(this.addTokenAddress, this.addTokenName, this.addTokenSymbol, this.addTokenDecimals)
        this.initTokens()
      ...
      initTokens() {
        const tokenList = [{ text: 'SIRX', value: 'SIRX' }]
        erc20.getTokenList().forEach((token) => {
