import siriusJsLib from 'siriusjs-lib'
import btcApp from '@ledgerhq/hw-app-btc'
import transportU2f from '@ledgerhq/hw-transport-u2f'
import BigNumber from 'bignumber.js'
import SafeBuffer from 'safe-buffer'
import OPS from 'siriusjs-lib'
...
export default class Ledger {
  ...
  static async connect() {
    const sirius = new btcApp(await transportU2f.create())
    // ensure in Sirius App
    const pubkeyRes = await sirius.getWalletPublicKey(Ledger.defaultPath)
    if (pubkeyRes.bitcoinAddress[0] !== 'Q') {
      throw 'Not Sirius App'
    }
    return new Ledger(sirius)
  }
  ...
  static async generateTx(keyPair, ledger, path, from, to, amount, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const pubkeyRes = await ledger.sirius.getWalletPublicKey(path)
    ...
    const inputs = []
    const paths = []
    const selectUtxo = siriusJsLib.utils.selectTxs(utxoList, amount, fee)
    const rawTxCache = {}
    for(let i = 0; i < selectUtxo.length; i++) {
      ...
      inputs.push([
        await ledger.sirius.splitTransaction(rawTxCache[item.hash]),
        item.pos
      ])
    }
    const outputs = new siriusJsLib.TransactionBuilder(keyPair.network)
    ...
    return await ledger.sirius.createPaymentTransactionNew(inputs, paths, undefined, outputsScript)
  }
  ...
  static async generateSendToContractTx(keyPair, ledger, path, from, contractAddress, encodedData, gasLimit, gasPrice, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const outputs = new siriusJsLib.TransactionBuilder(keyPair.network)
    const contract =  siriusJsLib.script.compile([
      OPS.OP_4,
